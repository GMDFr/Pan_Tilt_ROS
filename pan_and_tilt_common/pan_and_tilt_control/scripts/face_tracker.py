...
class FaceTracker(object):
    ...
    def face_follow(self, init_pan, init_tilt):
        """
        ...
        pan_tilt_pose = [init_pan, init_tilt]
...
def main():
    ...
    if len(sys.argv) > 3:
        init_pan = float(sys.argv[1])
        init_tilt = float(sys.argv[2])
    else:
        init_pan = 0.0
        init_tilt = 0.0

    rospy.loginfo("Init PAN,TILT=["+str(init_pan)+","+str(init_tilt)+"]")

    face_tracker_obj = FaceTracker()
    face_tracker_obj.face_follow(init_pan,init_tilt)
    rospy.spin()
