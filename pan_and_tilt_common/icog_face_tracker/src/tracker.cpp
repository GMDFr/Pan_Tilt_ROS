...
std::string default_camera_topic = "/default_camera_topic";
...
int main(int argc, char **argv) {
    face_cascade.load("/opt/ros/kinetic/share/OpenCV-3.3.1-dev/haarcascades/haarcascade_frontalface_alt.xml");
    ros::init(argc, argv, "tracker_node");
    ros::NodeHandle nh;
    nh.param("/tracker_node/show_preview", showPreview, false);
    nh.param("/tracker_node/camera_topic", camera_topic_name, default_camera_topic);
    pub = nh.advertise<icog_face_tracker::faces>("/faces", 5);
    ros::Subscriber sub = nh.subscribe(camera_topic_name, 1, imageCB);
    ros::spin();
}
